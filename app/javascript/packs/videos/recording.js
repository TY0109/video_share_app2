document.addEventListener('turbolinks:load', () => {
    ...
    function getWebCamera(webCamera) {
      ...
      .catch(e => alert('webカメラを取得できませんでした。'));
    ...
    function getScreenCapture(screenCapture, browserAudio) {
      ...
      .catch(e => alert('画面キャプチャを取得できませんでした。'));
    ...
    function getMicrophoneAudio(micAudio) {
      ...
      .catch(e => alert('マイク音声を取得できませんでした。'));
    ...
    function startRecording() {
      ...
      if (micAudioStream && browserAudioStream) {
        ...
        ms.addTrack(micAudioStream.getAudioTracks()[0]);
      } else if (browserAudioStream) {
        ms.addTrack(browserAudioStream.getAudioTracks()[0]);
      }
      ...
      } else if (micAudioStream || browserAudioStream) {
      ...
      try {
        ...
        alert('Exception while creating MediaRecorder: ' + error.message);
        return;
      }
      ...
      try {
        ...
        if (options === null) {
          alert('録画デバイスを反映してください。');
        } else {
          alert('error record ' + error.message);
        }
      }
    }
  
    // 録画停止ボタンクリックで発火 → 録画がストップするメソッド
    function stopRecording() {
      mediaRecorder.stop();
      console.log('Recorded media.');
    }
  
    // 録画開始or停止ボタンをクリックで発火 → 録画が開始or停止される
