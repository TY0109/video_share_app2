class CommentsController < ApplicationController
  ...
  before_action :ensure_admin_user_or_viewer
  ...
  def correct_admin_or_user_or_viewer_comment
    @comment = Comment.find(params[:id])
    set_video_id
    if @comment.system_admin_id != system_admin&.id || @comment.user_id != current_user&.id || @comment.viewer_id != current_viewer&.id
