...
RSpec.describe 'Replies', type: :request do
  ...
  let(:system_admin_reply) { create(:system_admin_reply, organization_id: user.organization_id, video_id: video_it.id) }
    ...
    describe 'システム管理者' do
      ...
      describe '正常' do
        it '返信がアップデートされる' do
          ...
          }.to change { Reply.find(user_reply.id).reply }.from(user_reply.reply).to('システム管理者のアップデート返信')
        end
  ...
  describe '動画視聴者' do
    ...
    describe '正常' do
      it '返信がアップデートされる' do
        ...
        }.to change { Reply.find(user_reply.id).reply }.from(user_reply.reply).to('動画視聴者のアップデート返信')
      end
